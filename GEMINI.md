...
**Choose the integration method based on the app's functional requirements:**

- **A) WebSocket** — Use when the app's feature involves ANY of these:
    - Chat bots, QA bots, or ANY conversational interface where the user and agent communicate back and forth.

    - Handling any long-running or multi-step tasks (e.g., heavy data processing, sequential workflows where the agent sends multiple events over time, report generation).

    - Document and PDF upload/extraction (e.g., parsing, analyzing, or extracting structured data from uploaded files).

    - Real-time / live feedback to the user (e.g., streaming agent responses, progress updates, live processing status).

    - **How to implement:** Follow `@./AGENT_INTEGRATION_WEB_SOCKET.md` and refer to `agentSdk/agents.ts` for config setup

- **B) API (emitter.emit)** — Use when the app's feature involves:
    - Synchronous request-response calls (e.g. send data, get result back immediately)
    - Simple one-shot agent triggers where you only need the final result
    - Fire-and-forget async events that don't need real-time feedback to the user
    - **How to implement:** Use `emitter.emit()` from `@/agentSdk` as documented in the sections below

**Decision rule:** Look at the app feature being built. If it involves chat bots, QA bots, any conversation, uploading PDFs/documents for extraction, streaming updates, progress indicators, or any long or multi-step wait, you MUST use **WebSocket**. If it's a simple, fast call-and-response or background trigger with no user-facing progress state, use **API (emitter.emit)**.
...
**Decide integration method from the app context being built:**
